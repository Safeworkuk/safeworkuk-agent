...
if (!OPENAI_API_KEY) {
  console.error("❌ Missing OPENAI_API_KEY. Add it in Render → Environment Variables.");
  process.exit(1);
}
...
async function embedTexts(texts) {
  const json = await openai('embeddings', { model: 'text-embedding-3-small', input: texts });
...
// ----------------- Start Server -----------------
app.listen(PORT, () => {
  console.log(`✅ SafeWorkUK Agent running on http://localhost:${PORT}`);
});
